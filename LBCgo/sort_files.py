...
arm_str = {'LBC-RED': 'red_', 'LBC_BLUE': 'blue_'}
filter_str = {'SDT_Uspec': 'Uspec',
              'g-SLOAN': 'SDSS_g',
              'r-SLOAN': 'SDSS_r',
              'i-SLOAN': 'SDSS_i',
              'z-SLOAN': 'SDSS_z',
...
if 1:
    ...
    if os.path.lexists('_sort_LBC.sav'):
        ...
        ascii.write([unused], 'sort_LBC_unused', overwrite=1, format='no_header')
...
if 1:
    ...
    for arm in biases:
        ...
        ascii.write([sorted(names)], biasdir + '/list',
                    overwrite=1, format='no_header')
...
if 1:
    ...
    for arm in flats:
        for filter in flats[arm]:
            ...
            ascii.write([sorted(names)], filtdir + '/list',
                        overwrite=1, format='no_header')
...
if 1:
    ...
    for obj in objects:
        objdir = 'sci_' + obj
        objdir = objdir.replace('/', '_').replace('\\', '_').replace(' ', '')
